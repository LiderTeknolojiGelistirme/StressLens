    ...
    public void DisconnectPluxDev()
    {
        if (AcquisitionThread != null)
        {
            ...
            if (AcquisitionStopped == false)
            {
                ...
                {
                    Console.WriteLine("StopAcquisitionUnity exception during disconnect: " + e.Message);
                }
            }
        }
        ...
        if (DeviceConnected)
        {
            ...
            {
                Console.WriteLine("DisconnectPluxDevUnity exception: " + e.Message);
            }
    ...
    public bool StopAcquisitionUnity(int forceStop=0)
    {
        ...
        if (AcquisitionThread != null)
        {
            if (forceStop >= 0)
            {
                ...
                // Wait for the communication of the flag stating the end of the communication loop (timeout ile).
                bool communicationFlag = GetCommunicationFlag();
                Console.WriteLine("Communication Flag (After Interrupt): " + GetCommunicationFlag());
                int maxWaitMs = 3000; // Maksimum 3 saniye bekle
                int waitedMs = 0;
                while (communicationFlag == true && waitedMs < maxWaitMs)
                {
                    Thread.Sleep(10);
                    waitedMs += 10;
                ...
                {
                    Console.WriteLine("Communication Flag timeout! Forcing stop after " + maxWaitMs + "ms");
                }
                ...
                {
                    Console.WriteLine("StopAcquisition exception: " + e.Message);
                }

                Console.WriteLine("Thread State: " + AcquisitionThread.ThreadState);
                // Thread'i güvenli bir şekilde kapat (Join ile timeout)
                if (AcquisitionThread.IsAlive)
                {
                    if (!AcquisitionThread.Join(2000)) // 2 saniye bekle
                    {
                        Console.WriteLine("Thread did not stop gracefully, aborting...");
                        AcquisitionThread.Abort();
                    }
                }
                Console.WriteLine("Thread State (After Stop): " + AcquisitionThread.ThreadState);
            }
            else
            {
                // Close Thread.
                ...
                if (AcquisitionThread.IsAlive)
                {
                    if (!AcquisitionThread.Join(2000))
                    {
                        AcquisitionThread.Abort();
                    }
                }
